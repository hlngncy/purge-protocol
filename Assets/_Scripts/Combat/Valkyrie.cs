...
public class Valkyrie : NetworkBehaviour, IAgent
{
    ...
    public static void OnBulletInstance(Changed<Valkyrie> changed)
    {
        changed.Behaviour._pool.GetProjectile(changed.Behaviour.barrelRef, Vector3.zero);
    }
    
    public static void OnAgentFired(Changed<Valkyrie> changed)
    {
